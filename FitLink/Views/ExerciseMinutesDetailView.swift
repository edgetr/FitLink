...
struct ExerciseMinutesDetailView: View {
    ...
    private var totalValueCard: some View {
        VStack(spacing: 12) {
            ...
            Text(viewModel.formattedExerciseMinutes)
                .font(.system(size: 48, weight: .bold))
                .monospacedDigit()
            
            Text("of exercise today")
                .font(.subheadline)
    ...
    private var hourlyChartSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if viewModel.hourlyExerciseMinutes.isEmpty {
                ...
                ScrollViewReader { proxy in
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(alignment: .bottom, spacing: 8) {
                            ForEach(viewModel.hourlyExerciseMinutes) { dataPoint in
                                VStack(spacing: 4) {
                                    if dataPoint.value > 0 {
                                        Text("\(Int(dataPoint.value))m")
                                    ...
                                    RoundedRectangle(cornerRadius: 4)
                                        ...
                                        .frame(width: 28, height: max(4, CGFloat(dataPoint.value) * 20))
